...
def supprimer_ligne(matrice_j,taille_c):
    global score, nb_lignes, nb_20lignes, vitesse, vitesse_max
    ...
    if nbr_ligne_effectuees >= 4: # un tetris vaut 1200 points
        ...
        screen.blit(tetris_affichage,(715, 500))
        pygame.display.flip()
        pygame.time.delay(1000) # petit délai pour lire le tetris

    # toutes les 20 lignes on accélère la vitesse tant qu'on a pas atteint la vitesse max
    if nb_lignes >= (nb_20lignes+1) * 3:
...
def refresh():
    ...
    score_affichage = police.render(f'Score : {score}', 1, (255,255,255) )
    screen.blit(score_affichage,(680, 120))
    ...
    next_affichage = police.render(f'NEXT', 1, (255,255,255) )
    screen.blit(next_affichage,(680, 220))
    screen.blit(rect_prochaine_piece, (680, 265))
...
def affichage_pause():
    """ rôle : mettre le jeu en pause
        ...
        Sortie : None
    """
    global manette1, bouton_pause, dernier_deplacement,delai_deplacement
...
def reset():
    """ rôle : relancer le jeu
        ...
        Sortie : None
    """
    global matrice_jeu, score, vitesse, nb_20lignes, nb_lignes
...
while running:
    play = True
    perdu = False
    # on vérifie que le joueur n'a pas déconnecté sa manette en milieu de partie
    nb_manettes = pygame.joystick.get_count()
    ...
    if play:
        ...
        prochaine_piece = Carre(couleur_prochaine_piece, 700, 238-55*hauteur_proch_piece+100, prochaine_piece_forme)
        ...
        while play and not perdu:
            ...
            if nb_manettes >= 1 :
                ...
                if time.time() - dernier_deplacement > delai_gauche_droite:
                ...
                # délai pour les mouvements haut/rotation
                if time.time() - dernier_deplacement > delai_deplacement:
                    ...
                    if pygame.JOYHATMOTION:
                        ...
                        if croix[1] < 0 and piece_actuelle.y + len(piece_actuelle_forme) * piece_actuelle.hauteur - piece_actuelle.hauteur + 6 <= 638:
                            piece_actuelle.deplacer(0, vitesse + 5) # Fait chuter de +5y tout le long que l'on presse la fleche du bas
                            for ligne in matrice_jeu :
                                    if any(piece_actuelle.collision(piece) for piece in ligne):
                                        piece_actuelle.deplacer(0, -vitesse -5 +1)
                    ...
                    if pygame.JOYAXISMOTION:
                        ...
                        if axe_y >= 0.5 and piece_actuelle.y + len(piece_actuelle_forme) * piece_actuelle.hauteur - piece_actuelle.hauteur + 6 <= 638:
                            piece_actuelle.deplacer(0, vitesse + 5) # Fait chuter de +6y tout le long que l'on presse la fleche du bas
                            for ligne in matrice_jeu :
                                    if any(piece_actuelle.collision(piece) for piece in ligne):
                                        piece_actuelle.deplacer(0, -vitesse -5+1)
            ...
            # s'il n'y a pas de manette de détectée
                ...
                # Délai pour mouvements haut/rotations
                if time.time() - dernier_deplacement > delai_deplacement:
                    ...
                    if keys[pygame.K_DOWN] and piece_actuelle.y + len(piece_actuelle_forme) * piece_actuelle.hauteur - piece_actuelle.hauteur + 6 <= 638:
                        ...
                        for ligne in matrice_jeu :
                                if any(piece_actuelle.collision(piece) for piece in ligne):
                                    piece_actuelle.deplacer(0, -vitesse -5)
    ...
    if perdu:# si on a perdu
        ...
        pygame.time.delay(2500) # petit délai pour lire le texte pour rejouer
        ...
        while attente_de_touche:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN or pygame.JOYBUTTONDOWN :
