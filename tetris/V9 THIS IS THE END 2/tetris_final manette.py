...
class Carre:
    ...
    def descendre_matrix(self, piece_forme , matrice_j):
        ...
        if self.y + len(piece_forme) * self.hauteur - self.hauteur >= 638:
            # On retourne aussi les coordonnées d'origine
            self.x = ancien_x
            self.y = ancien_y
            self.rectangle = pygame.Rect(self.x, self.y, self.largeur, self.hauteur)
...
def refresh():
    ...
    screen.blit(niveau_affichage,(680, 220))
    ...
    next_affichage = police.render(f'NEXT', 1, (255,255,255) )
    screen.blit(next_affichage,(680, 270))
    screen.blit(rect_prochaine_piece, (680, 315))
...
while running:
    ...
    if play:
        ...
        prochaine_piece = Carre(couleur_prochaine_piece, 700, 238-55*hauteur_proch_piece+145, prochaine_piece_forme)
        ...
        while play and not perdu:
            ...
            if nb_manettes >= 1 :
                    ...
                    # délai pour les mouvements haut
                    if time.time() - dernier_deplacement > delai_deplacement:
