...
choix_nbmikados = pygame.image.load("bouton nbmikados.png")
...
def refresh():
    """ rôle : afficher les éléments
        ...
        Sortie : None
    """
    global screen, fond, bouton_quitte, bouton_regles, screen_x, screen_y,batons_restants, nb_mikados, dict_mikados, tour_joueur, bouton_nombre
...
# chosir le nombre de mikados en début de partie
def choix_mikados():
    """ rôle : choisir le nombre de mikados pour la partie
        Entrées : None
        Sortie : int : le nombre de mikados
    """
    global screen, police,fond, choix_nbmikados, screen_x

    # on affiche le texte pour choisir le nombre de mikados
    texte_choix1 = police.render(f"Choisir le nombre de mikados", 1, (255,255,255) )
    texte_choix2 = police.render(f"pour la partie", 1, (255,255,255) )

    screen.blit(texte_choix1,(150, 150))
    screen.blit(texte_choix2,(350, 220))

    # on affiche les boutons de choix
    screen.blit(choix_nbmikados,(200, 400))
    pygame.display.flip()

    choisi = False

    # tant que le joeur n'a pas choisi on attend
    while not choisi:
        for event in pygame.event.get():

            # si il a cliqué sur la croix
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # si il a cliqué sur le bouton 8
                if 289 <= co[0] <= 474 and 435 <= co[1] <= 636:
                    # le nombre de mikados pour la partie est égal à 8
                    nombre = 8
                    choisi = True

                # si il a cliqué sur le bouton 15
                elif 560 <= co[0] <= 745 and 435 <= co[1] <= 635:
                    # le nombre de mikados pour la partie est égal à 15
                    nombre = 15
                    choisi = True
    return nombre
...
# on choisi le nombre de mikados
nb_mikados = choix_mikados()
...
while running:
    ...
    if jeu_termine:# si le jeu est fini
        ...
        if gagne:
            if nb_manches_jouees - manches_gagnees != 8 :
                if manches_gagnees != 8:
        ...
        elif nb_manches_jouees - manches_gagnees != 8:
        ...
        # si on relance la partie
        if nb_manches_jouees == 0:
            # on choisi le nombre de mikados
            nb_mikados = choix_mikados()
