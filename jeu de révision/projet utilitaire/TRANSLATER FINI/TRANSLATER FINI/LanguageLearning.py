...
def random_word():
    ...
    """
    global liste_revision, liste_traduction, mot_corrige, mot_aleatoire, nombre_de_questions
    
    nombre_de_questions = 252 - len(liste_traduction)
...
def afficher_score_tkinter():
    global prenom, score, langue_revision, langue_traduction, fenetre_score

    fenetre_score = tk.Tk()
    fenetre_score.title("Scores")
    fenetre_score.geometry("%dx%d%+d%+d" % (600, 600, (fenetre_score.winfo_screenwidth()-600)//2, (fenetre_score.winfo_screenheight()-600)//2))
    ...
    fenetre_score.protocol("WM_DELETE_WINDOW", affichage_graph)
    fenetre_score.mainloop()

def afficher_score():
    global prenom, score, langue_revision, langue_traduction, nombre_de_questions
    if nombre_de_questions == 0:
        moyenne = 0
    else:
        moyenne = score * 20 / nombre_de_questions

    with open("tableau_score.csv", 'a', newline='') as fichier_csv:
        fichier = csv.writer(fichier_csv, delimiter=';', lineterminator="\n")
        fichier.writerow([moyenne])

    with open("score.txt", "a", encoding='utf-8') as fichier:
        fichier.write(f"Nom : {prenom}, Score : {score} / {nombre_de_questions}, Langue de révision : {langue_revision}, Langue de traduction : {langue_traduction}\n")
...
def lire_fichier_score_csv():
    ...
    with open("tableau_score.csv", newline='', encoding='iso-8859-1') as fichier_csv:
        fichier = csv.reader(fichier_csv, delimiter=';')
        for ligne in fichier:
            resultat.append(ligne[0])  # on ajoute la moyenne de points sur 20 à la liste
...
def affichage_graph():
    ...
    if len(moyennes_score) <= 20:  # si il y a 20 moyennes enregistrées ou moins
        moyennes_recentes = moyennes_score[:]  # on prend toutes les moyennes
    else:  # sinon
        moyennes_recentes = moyennes_score[len(moyennes_score)-20:]  # on prend les 20 moyennes les plus récentes

    moyennes_recentes = inverse(moyennes_recentes)  # on inverse le contenu de la liste pour afficher du plus au moins récent

    x = [n+1 for n in range(len(moyennes_recentes))]  # les valeurs des abscisses
    y = [float(m) for m in moyennes_recentes]  # les valeurs des ordonnées

    # Vérifiez la taille des listes x et y
